 ...
 * @param {boxShadowPassive} boxShadowPassive: boxShadowPassive styling applied to all elements in this class. Default is '0px 0px 1px 1px #555f'
 */
function Interactable(boxShadowACtive = null, boxShadowPassive = null) 
{
    //box-shadow: oofset-x, offset-y, blur-radius, spread-radius
    let BoxShadowActive = boxShadowACtive ?? '0px 0px 20px 4px #aaaf';
    let BoxShadowPassive = boxShadowPassive ?? '0px 0px 1px 1px #aaaf';
    ...
    function loadResizables(BoxShadowPassive){
        ...
        for (const elem in Elems)
        {
            ...
            {
                if (BoxShadowPassive != 'none') Elems[elem].style['box-shadow'] = BoxShadowPassive;
                Elems[elem].style['position'] = "relative";
                Elems[elem].style['overflow'] = "clip";
            }
        }
        ...
        for (const elem in resizables)
        {
            ...
            {
                let el = resizables[elem].appendChild(resizeElem.cloneNode(true));
        ...
        for (const elem in slidables)
        {
            ...
            {
                let el = slidables[elem].appendChild(slideElem.cloneNode(true));
                el.addEventListener("mousedown", handleMouseDown);
                el.addEventListener("touchstart", handleMouseDown,{passive: false});
            }
        }
        ...
        for (const elem in moveables) {
            if (Object.hasOwnProperty.call(moveables, elem)) {
                moveables[elem].addEventListener("mousedown", handleMouseDown);
                moveables[elem].addEventListener("touchstart", handleMouseDown,{passive: false});
                moveables[elem].style['cursor'] = "all-scroll";
            }
        }
        ...
        for (const elem in dockables) {
            if (Object.hasOwnProperty.call(dockables, elem)) {
                ...
                let el = dockables[elem].appendChild(dockElem.cloneNode(true));
                el.addEventListener("dblclick", handleDblClick);

                if (dockables[elem].classList.contains('docked')) dockables[elem].style['position']="relative"
                else if (dockables[elem].classList.contains('undocked')) dockables[elem].style['position']="absolute";

                if (!dockables[elem].style['top'])
                {
                    dockables[elem].style['top'] = window.getComputedStyle(dockables[elem], "top");
                    if (dockables[elem].style['top'] == "")
                    {
                        //init styling.top
                        if (dockables[elem].classList.contains('docked')) dockables[elem].style['top'] = Number(dockables[elem].getClientRects()['top']).toFixed(2) + "px";
                        else if (dockables[elem].classList.contains('undocked')) dockables[elem].style['top'] = Number(dockables[elem].getBoundingClientRect()['top']).toFixed(2) + "px";
                    }
                }
            }
        }
        
        //alert("loaded interactables.js");
    }
    loadResizables(BoxShadowPassive);
    ...
    function handleMouseUp(event)
    {
        ...
        if (Elem) {
            if (BoxShadowPassive != 'none') Elem.style["box-shadow"] = BoxShadowPassive;
            Elem = undefined;
        }
    ...
    function bringElemToTop()
    {
        ...
        if (BoxShadowActive != 'none') Elem.style["box-shadow"] = BoxShadowActive;

        for (const el in Elems) {
            if (Object.hasOwnProperty.call(Elems, el)) {
                if (Elem != Elems[el]) Elems[el].style["z-index"] = 0;   //bring to bottom
            }
        }
    }
    ...
    function move(event)
    {
        ...
        {
            Elem.style["left"] = Number(Left).toFixed(3) + "px";
        }


        if (Elem.classList.contains("cloneable"))
        {
            let els = document.elementsFromPoint(clientX, clientY);
            

            for (const el in els)
            {
                if (els[el].classList.contains("birthable"))
                {
                    //alert("Birth " + Elem.id + " a clone?");

                    let baby = Elem.cloneNode(true);

                    els[el].appendChild(baby);
